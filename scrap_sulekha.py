...
print(cities_li)
'''
cities_li = [u'tirupati', u'vijayawada', u'visakhapatnam', u'patna', u'delhi', u'faridabad', u'goa', u'ahmedabad', 
			u'jamnagar', u'rajkot', u'surat', u'vadodara', u'gurgaon', u'jamshedpur', u'bangalore', u'hubli', u'mangalore', 
			u'calicut', u'cochin', u'thrissur', u'trivandrum', u'bhopal', u'indore', u'aurangabad', u'kolhapur', u'mumbai', 
			u'nagpur', u'nashik', u'pune', u'pune', u'bhubaneswar', u'amritsar', u'chandigarh', u'jalandhar', u'ludhiana', 
			u'jaipur', u'chennai', u'coimbatore', u'madurai', u'trichy', u'hyderabad', u'agra', u'allahabad', u'ghaziabad', 
			u'kanpur', u'lucknow', u'noida', u'varanasi', u'noida', u'varanasi', u'kolkata']

#############################################################################################################################################

city = 'bangalore'
link_mod_li = [u'administration-training-centres']

url_temp = 'http://yellowpages.sulekha.com/computer-training-centres_'+city+'_clistings'
#url_temp = 'http://yellowpages.sulekha.com/colleges-universities_'+city+'_clistings'
#url_temp = 'http://yellowpages.sulekha.com/schools_'+city+'_clistings'

headers = {'User-agent': 'Mozilla/5.0'}
webpage = requests.get( url_temp, headers=headers )
soup = BeautifulSoup(webpage.content, "html.parser")

temp_div_li = soup.find_all('div',class_="blockTitle blockarrow")

for x in temp_div_li:
	ss = x.find('a')['href']
	#print(x)
	#print(ss)
	link_mod_li.append(ss[ss.find('.com/')+5:ss.find('_')])

temp_div_li = soup.find_all('div',class_="blockTitle")
...
for city in cities_li:
	...
	with open('/home/4r0r4/Desktop/data/sulekha/scrap2_'+city+'.csv', 'w') as mycsv:
		c = csv.writer(mycsv, dialect='mydialect')	
		
		c.writerow(['Name','Main Category','Phone1','Phone2','Phone3','Phone4','Phone5','Street Address','Locality','Pincode','City','Country',
		...
		for link_mod in link_mod_li:
			try : 

				while(True):
					#http://yellowpages.sulekha.com/filmmaking-schools_kolkata
					if link_mod == 'arts-science-colleges' or link_mod == 'business-colleges' :
						break
					print(link_mod)
					print('###########################################')
					 
					url1 = 'http://yellowpages.sulekha.com/'+link_mod+'_'+city + '_'+ str(temp_count)
					...
					ee =  0
					if soup.find('li', class_ = 'next') is None : ee = 1
				...
				c.writerow(['1234567890'])
				continue
	'''
	with open('/home/4r0r4/Desktop/data/sulekha/scrap_'+city+'.csv', 'rb') as mycsv_r:
		c_read = csv.reader(mycsv_r, dialect='mydialect')
		row_cnt = 0
		with open('/home/4r0r4/Desktop/data/sulekha/scrap_'+city+'_'+str(row_cnt)+'-'+str(row_cnt+1000)+'.csv', 'w') as mycsv_w:
		for row in c_read:
			row_cnt += 1
				
			c_write = csv.writer(mycsv_w, dialect='mydialect')
			c_write.writerow(row)

			if row_cnt % 10 == 0 : goto .A
	'''
